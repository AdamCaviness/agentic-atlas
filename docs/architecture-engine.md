   ...
   spec.load_rubric ──▶ models.Rubric (Axis[] → Indicator[])
        ...
        profiler.profile_target                     ← the SINGLE code path
          for each axis, for each indicator:
            ├─ DETECTED  → evidence.resolve_detected(ind, target)
            └─ JUDGED→ judged.resolve_judged(ind, target, answers)
...
each to the detected or judged resolver, scores each axis, and stamps the profile with
...
5. **`detected` vs `judged` stay separate** — two resolvers, never blurred; the engine
   validates judged answers, it never produces them.
...
and `test_judged.py` cover the two resolvers including the honest-coverage guards;
