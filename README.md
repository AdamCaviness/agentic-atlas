...
- It is **not a leaderboard**, with no ranking and no aggregate score. Collapsing independent axes into one number is meaningless, and picking a single winner is the wrong question anyway: you want the tool that fits your project and the way you work, not a universal "best."
...
- The rubric is deliberately **opinionated**, versioned under semver, and open to challenge. When you disagree with where a tool landed, you can trace the position to the exact indicators behind it and open a pull request to change them.
